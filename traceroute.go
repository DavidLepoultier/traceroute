...
func socketAddr(sAddr string) (addr [4]byte, err error) {
	...
	for _, a := range addrs {
		if ipnet, ok := a.(*net.IPNet); ok && !ipnet.IP.IsLoopback() {
			if sAddr != "" {
				if len(ipnet.IP.To4()) == net.IPv4len && ipnet.IP.String() == sAddr {
...
//
func getSourceAddr() (net.IP, error) {
	...
	return nil, fmt.Errorf("Could not find a source address in af %s", "ip4")
}
...
func Traceroute(dest string, options *TracerouteOptions, scoksAddr string, c ...chan TracerouteHop) (result TracerouteResult, err error) {
	result.Hops = []TracerouteHop{}
	destAddr, err := destAddr(dest)
	result.DestinationAddress = destAddr
	socketAddr, err := socketAddr(scoksAddr)
	if err != nil {
		return
	}
	...
	for {
		...
		//syscall.SetsockoptInt(sendSocket, syscall.SOL_SOCKET, syscall.IP_TOS, 128)
		...
		syscall.Connect(sendSocket, &syscall.SockaddrInet4{Port: options.Port(), Addr: destAddr})
